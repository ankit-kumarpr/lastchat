...
exports.registerUser = async (req, res) => {
  try {
    ...
    const user = new User({ name, email, password: hashed, customerId, phone: phone || '' });
    await user.save();
...
exports.login = async (req, res) => {
  try {
    ...
    res.json({
      ...
      token, user: { id: user._id, name: user.name, email: user.email, phone: user.phone, role: user.role, customerId: user.customerId } });
  } catch (err) {
    console.error(err);
    res.status(500).json({ message: 'Server error' });
  }
...
exports.registerAdmin = async (req, res) => {
  try {
    ...
    if (adminSecret !== process.env.ADMIN_CREATION_SECRET) return res.status(403).json({ message: 'Not allowed' });

    const existing = await User.findOne({ email });
    if (existing) return res.status(400).json({ message: 'Email already exists' });
    const hashed = await bcrypt.hash(password, 10);

    const admin = new User({ name, email, password: hashed, role: 'admin', customerId: await ensureUniqueCustomerId() });
    await admin.save();
